...
function validateSPName(isRegister) {
    ...
    if( spName.length == 0) {
        ...
        message({content:'Please provide Service Provider ID', type:'error', cbk:function () {} });
        return false;
    } else {
        if(isRegister) {
            registerCustomSP();
...
function preDrawSAMLConfigPage(){
    $.ajax({
        ...
        error: function (e) {
            message({content: 'Error occurred while getting the service provider configuration.', type: 'error', cbk: function () {
            }});
        }
    });
}
...
function addAudienceFunc(chkbx) {
    document.addServiceProvider.audience.disabled = (chkbx.checked) ? false
        : true;
    document.addServiceProvider.addAudience.disabled = (chkbx.checked) ? false
        : true;
